...
public class Tagger
{
   ...
   */
  private final int lcontextWindow = 3;
  private final int rcontextWindow;
  ...
  public Tagger(File modelFolder, int maxSentenceLength, double beta, boolean nolookahead) {
    try {
      FilenameFilter embeddingsFileFilter = new PatternFilenameFilter("embeddings.*");

      rcontextWindow = (nolookahead) ? 0 : 3;
      ...
      totalFeatures = 
        (embeddingsFeatures.get(unknownLower).length + 
            ...
            discreteFeatures.get(capsLower).length)*(lcontextWindow + rcontextWindow + 1);
  ...
  public List<List<ScoredLabel>> tag(List<InputWord> words) {
    final List<List<ScoredLabel>> result = new ArrayList<>(words.size());
    final double[] vector = new double[totalFeatures];
    
    for (int wordIndex = 0; wordIndex < words.size(); wordIndex++) {
      int vectorIndex = 0;
      for (int sentencePosition = wordIndex - lcontextWindow; sentencePosition<= wordIndex+rcontextWindow; sentencePosition++) {
  ...
  /**
   * Returns a list of @ScoredLabel for this word, sorted by their probability. 
   * @param vector A vector
   * @param word The word itself.
   * @param wordIndex The position of the word in the sentence.
   * @return
   */
  private List<ScoredLabel> getTagsForWord(final Vector vector, final InputWord word, final int wordIndex)
  {
    double total = 0.0;
    ...
    // Convert the queue into a sorted list of ScoredLabels.
    List<ScoredLabel> result = new ArrayList<>(queue.size());
    while (queue.size() > 0) {
      ScoredCategory cat = queue.poll();
      double probability = cat.score / total;
      result.add(new ScoredLabel(Label.make(lexicalCategories.get(cat.id).toString()), probability)); 
    }
