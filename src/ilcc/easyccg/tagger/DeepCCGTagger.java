...
import ilcc.easyccg.tagger.EasyCCGTagger.InputFormat;
...
public class DeepCCGTagger
{
  
    private Tagger tagger;
    private double corr, ncorr, total;
    
    public DeepCCGTagger(File model, int maxLen, double beam, boolean nolookahead){
  ...
  public static void main(String[] args) throws IOException, ArgumentValidationException, Exception { 
      ...
      String model;
      if(args.length <1)
        model = "/home/ambati/ilcc/tools/deeptagger/model";
      else
          model = args[0];
      DeepTagger tagger = DeepTagger.make(new File(model));
      List<String> tagList = tagger.getTags();
      List<String> words = Arrays.asList("Marcel", "proved", "completeness");
      float[][] scores = tagger.tag(words);
      System.err.println("Score for " + words.get(1) + " with tag " + tagList.get(2) + " is: " + scores[1][2]);
    ...
    DeepCCGTagger etagger = new DeepCCGTagger(parsedArgs.getModel(), parsedArgs.getMaxLength(), parsedArgs.getSupertaggerbeam(), parsedArgs.getNolookahead());
