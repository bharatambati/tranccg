...
public class Classifier {
  ...
  double[] computeScores(int[] feature, List<Integer> olist) {
  ...
  private double[] computeScores(int[] feature, Map<Integer, Integer> preMap, List<Integer> olist) {
    ...
    for (int i = 0; i < config.hiddenSize; ++i) {
      hidden[i] += b1[i];
      if(config.activation.equals("relu"))
          hidden[i] = Math.max(0.0, hidden[i]);           // ReLU
      else
...
/*
  ...
  public double[] computeScoresY(double[] hidden, List<Integer> olist) {
    int size = olist.size();
    double[] scores = new double[size];
    for (int i = 0; i < size; ++i)
      for (int j = 0; j < config.hiddenSize; ++j)
